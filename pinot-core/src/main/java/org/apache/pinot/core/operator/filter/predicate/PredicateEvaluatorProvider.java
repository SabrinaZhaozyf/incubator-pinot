  ...
  public static PredicateEvaluator getPredicateEvaluator(Predicate predicate, @Nullable Dictionary dictionary,
    ...
    try {
      if (dictionary != null) {
        ...
        switch (predicate.getType()) {
          case EQ:
            predicateEvaluator = EqualsPredicateEvaluatorFactory.newRawValueBasedEvaluator((EqPredicate) predicate, dataType);
            break;
          case NOT_EQ:
            predicateEvaluator = NotEqualsPredicateEvaluatorFactory.newRawValueBasedEvaluator((NotEqPredicate) predicate, dataType);
            break;
          case IN:
            predicateEvaluator = InPredicateEvaluatorFactory.newRawValueBasedEvaluator((InPredicate) predicate, dataType);
            break;
          case NOT_IN:
            predicateEvaluator = NotInPredicateEvaluatorFactory.newRawValueBasedEvaluator((NotInPredicate) predicate, dataType);
            break;
          case RANGE:
            predicateEvaluator = RangePredicateEvaluatorFactory.newRawValueBasedEvaluator((RangePredicate) predicate, dataType);
            break;
          case REGEXP_LIKE:
            predicateEvaluator = RegexpLikePredicateEvaluatorFactory
